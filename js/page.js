...
const bridgeEnabledInput = document.getElementById('bridgeEnabled')
const bridgeEndpointInput = document.getElementById('bridgeEndpoint')
const bridgeOutputFormatSelect = document.getElementById('bridgeOutputFormat')
const bridgeTimeoutSecInput = document.getElementById('bridgeTimeoutSec')
...
function readStyleFromUI () {
  return {
    ...
    bridgeOutputFormat: bridgeOutputFormatSelect.value,
    bridgeTimeoutSec: bridgeTimeoutSecInput.value
  }
}
...
function applyStyleToUI (style) {
  ...
  if (style.bridgeEnabled !== undefined) bridgeEnabledInput.checked = style.bridgeEnabled
  if (style.bridgeEndpoint !== undefined) bridgeEndpointInput.value = style.bridgeEndpoint
  if (style.bridgeOutputFormat !== undefined) bridgeOutputFormatSelect.value = style.bridgeOutputFormat
  if (style.bridgeTimeoutSec !== undefined) bridgeTimeoutSecInput.value = style.bridgeTimeoutSec
}
...
function getDefaultStyle () {
  return {
    ...
    particleEffect: 'none',
    bridgeEnabled: false,
    bridgeEndpoint: 'http://127.0.0.1:47890/transcode',
    bridgeOutputFormat: 'mp4',
    bridgeTimeoutSec: '180'
  }
}
...
bridgeEnabledInput.addEventListener('change', onStyleChange)
bridgeEndpointInput.addEventListener('input', onStyleChange)
bridgeOutputFormatSelect.addEventListener('change', onStyleChange)
bridgeTimeoutSecInput.addEventListener('input', onStyleChange)
...
exportBtn.addEventListener('click', async () => {
  ...
  if (bridgeEnabledInput.checked && !bridgeEndpointInput.value.trim()) {
    return notify('请填写本机软件接口地址')
  }
    ...
    await new Promise(resolve => {
      ...
      chrome.runtime.sendMessage({
        ...
        payload: {
          ...
          softwareBridge: {
            enabled: !!task.style.bridgeEnabled,
            endpoint: (task.style.bridgeEndpoint || '').trim(),
            outputFormat: task.style.bridgeOutputFormat || 'mp4',
            timeoutSec: Math.max(10, parseInt(task.style.bridgeTimeoutSec, 10) || 180)
          },
          batchIndex: i,
...
chrome.runtime.onMessage.addListener(msg => {
  ...
  if (msg.type === 'bridge-status-update') {
    if (msg.stage === 'processing') {
      document.getElementById('statusHint').textContent = '本机软件转码中...'
    } else if (msg.stage === 'done') {
      const outFmt = (msg.outputFormat || '').toUpperCase() || '目标格式'
      document.getElementById('statusHint').textContent = `本机软件转码完成（${outFmt}）`
    }
  }

  if (msg.type === 'bridge-failed') {
    notify(`本机转码失败，已自动回退为 WebM：${msg.message || '未知错误'}`)
    document.getElementById('statusHint').textContent = '本机转码失败，已回退为 WebM 下载'
  }
})
