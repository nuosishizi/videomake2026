...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  ...
  if (msg.type === 'export-done') {
    ...
    const safeExt = String(msg.fileExt || 'webm').toLowerCase().replace(/[^a-z0-9]/g, '') || 'webm'
    const filename = `video_${batchIndex + 1}_${Date.now()}.${safeExt}`
  ...
  if (msg.type === 'export-bridge-status') {
    chrome.runtime.sendMessage({
      type: 'bridge-status-update',
      stage: msg.stage,
      outputFormat: msg.outputFormat
    })
  }

  if (msg.type === 'export-bridge-failed') {
    chrome.runtime.sendMessage({
      type: 'bridge-failed',
      message: msg.message
    })
