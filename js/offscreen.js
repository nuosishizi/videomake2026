...
const MIN_OUTPUT_SHORT_SIDE = 1080
const TARGET_FPS = 30
...
/**
 * @description 转为偶数像素，避免编码器对奇数尺寸不兼容
 * @param {number} value
 * @returns {number}
 */
...
/**
 * @description 计算输出分辨率，默认保证短边至少 1080
 * @param {number} sourceWidth
 * @param {number} sourceHeight
 * @returns {{width:number,height:number,scale:number}}
 */
function getTargetOutputSize (sourceWidth, sourceHeight) {
  const safeW = Math.max(1, Number(sourceWidth) || 1)
  const safeH = Math.max(1, Number(sourceHeight) || 1)
  const shortSide = Math.min(safeW, safeH)
  const scale = Math.max(1, MIN_OUTPUT_SHORT_SIDE / shortSide)

  return {
    width: toEvenPx(safeW * scale),
    height: toEvenPx(safeH * scale),
    scale
  }
}

/**
 * @description 依据分辨率估算高质量 VP9 码率
 * @param {number} width
 * @param {number} height
 * @param {number} fps
 * @returns {number}
...
function estimateVideoBitrate (width, height, fps) {
  ...
  const estimated = Math.round(pixels * fps * 0.22)
  return Math.max(12_000_000, Math.min(45_000_000, estimated))
}
...
/**
 * @description 根据 mime 类型推断文件后缀
 * @param {string} mimeType
 * @param {string} fallbackFormat
 * @returns {string}
 */
function inferFileExtension (mimeType, fallbackFormat = 'webm') {
  const normalized = (mimeType || '').toLowerCase()
  if (normalized.includes('mp4')) return 'mp4'
  if (normalized.includes('webm')) return 'webm'
  if (normalized.includes('quicktime') || normalized.includes('mov')) return 'mov'
  return fallbackFormat === 'mp4' ? 'mp4' : 'webm'
}

/**
 * @description 调用本机软件接口进行二次转码
 * @param {Blob} sourceBlob - 当前导出的 WebM
 * @param {Object} bridgeConfig - 软件接口配置
 * @param {boolean} bridgeConfig.enabled - 是否启用接口
 * @param {string} bridgeConfig.endpoint - 接口地址
 * @param {string} bridgeConfig.outputFormat - 目标格式
 * @param {number} bridgeConfig.timeoutSec - 超时秒数
 * @param {number} batchIndex - 批次索引
 * @returns {Promise<{blob: Blob, fileExt: string, mimeType: string}>}
 */
async function transcodeBySoftwareBridge (sourceBlob, bridgeConfig, batchIndex) {
  ...
  if (!endpoint) {
    throw new Error('未配置本机软件接口地址')
  }

  const outputFormat = bridgeConfig.outputFormat === 'webm' ? 'webm' : 'mp4'
  const timeoutSec = Math.max(10, Math.min(3600, Number(bridgeConfig.timeoutSec) || 180))
  const controller = new AbortController()
  const timerId = setTimeout(() => controller.abort(), timeoutSec * 1000)

  chrome.runtime.sendMessage({
    type: 'export-bridge-status',
    stage: 'processing',
    outputFormat
  })
  ...
  try {
    ...
    formData.append('file', sourceBlob, `input_${batchIndex + 1}.webm`)
    ...
    try {
      ...
      if (error?.name === 'AbortError') {
        throw new Error(`本机接口超时（>${timeoutSec}s）`)
      }
      throw error
    }
    ...
    if (!resp.ok) {
      let detail = ''
      try {
        detail = (await resp.text()).slice(0, 1200)
      } catch {}
      throw new Error(`接口响应异常(${resp.status}) ${detail}`.trim())
    }

    const convertedBlob = await resp.blob()
    if (!convertedBlob.size) {
      throw new Error('接口返回空文件')
    }

    const mimeType = convertedBlob.type || (outputFormat === 'mp4' ? 'video/mp4' : 'video/webm')
    const fileExt = inferFileExtension(mimeType, outputFormat)
 ...
 * @param {Object} [payload.softwareBridge] - 本机软件接口配置
...
async function startExport (payload) {
  ...
  // 默认输出至少 1080 短边，提升清晰度
  const outputSize = getTargetOutputSize(sourceWidth, sourceHeight)
  ...
  console.log(`输出分辨率: ${canvas.width}x${canvas.height} (源 ${sourceWidth}x${sourceHeight}, 缩放 ${outputSize.scale.toFixed(2)}x)`)
  ...
  const videoBitrate = estimateVideoBitrate(canvas.width, canvas.height, FPS)
  console.log(`编码参数: ${FPS}fps, bitrate=${Math.round(videoBitrate / 1_000_000)}Mbps`)
  ...
  if (payload.softwareBridge && payload.softwareBridge.enabled) {
    try {
      const bridgeResult = await transcodeBySoftwareBridge(
        blob,
        payload.softwareBridge,
        payload.batchIndex !== undefined ? payload.batchIndex : 0
      )
      finalBlob = bridgeResult.blob
      fileExt = bridgeResult.fileExt
      mimeType = bridgeResult.mimeType
    } catch (error) {
      console.error('本机软件转码失败，已回退 WebM', error)
      chrome.runtime.sendMessage({
        type: 'export-bridge-failed',
  ...
  console.log('完成，大小:', (finalBlob.size / 1024 / 1024).toFixed(2), 'MB')
